...
- Five Gradle modules as of Milestone 8.1: `order-service`, `inventory-service`,
  `llm-fundamentals`, `common`, and `dispute-service`. `common` is the exception to
  "every subproject is a Spring Boot application" — it has no main class, `bootJar` is
  disabled and plain `jar` enabled instead (ADR-0006); it exists to be depended on, not
  run.
...
**Phase 7 (Vector and Retrieval Foundations) — Milestones 7.1 and 7.2 complete.**
`llm-fundamentals` gained a new package, `com.distributedplatform.llmfundamentals.vector`
— a deliberate package (not module) boundary, since this is a logical grouping within
existing Track B exploratory code, not a demonstrated architectural boundary (see
`ROADMAP.md` Milestone 7.1 for the reasoning). `VectorMath` implements `dotProduct`,
`magnitude`, and `cosineSimilarity` from scratch — pure Java, no Spring, no external
math library, unit-tested only, zero dependency on any embedding model or vector store
yet. Three named unchecked exceptions (`EmptyVectorException`,
`VectorDimensionMismatchException`, `ZeroVectorException`) represent three genuinely
distinct invalid-input conditions rather than one generic exception (ADR-0008).

A sibling package, `com.distributedplatform.llmfundamentals.embedding`, calls a real
local embedding model (`nomic-embed-text` via Ollama's `POST /api/embed`, 768
dimensions, empirically confirmed to normalize its output to ~unit magnitude) and does
brute-force retrieval over a small in-memory `List<Document>` — no persistence, no
approximate-nearest-neighbor index, both deliberately deferred to Phase 8. A real
end-to-end test embedding 8 real sentences and ranking them against a real query
proved the whole pipeline composes correctly: `VectorMath`'s hand-verified math (7.1)
plus a real embedding model (7.2) cleanly separated two unrelated topics by cosine
similarity. `OllamaEmbeddingService` mirrors `GeminiLlmClient`'s two-constructor shape
